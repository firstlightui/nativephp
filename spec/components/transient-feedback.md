...
Current executable evidence covers the PHP facade/store/events, child-registry callbacks, navigation refresh, event ordering, queue/tombstones, platform timing, lifecycle suspension, accessibility semantics, init registration, large-text and RTL test cases, and API 29 compilation. Snapshot suites and committed goldens cover tones, actions, held feedback, long copy, large text, and RTL, but only a fresh passing `verifyPaparazziDebug` establishes current Android visual-regression evidence. The registered showcase route is `/captures/transient-feedback` with a focused `TransientFeedbackCaptureTest.php` command and four standard image outputs.

The showcase fixture, runtime screenshot matrix, simulator/emulator execution, VoiceOver, TalkBack, and physical-device review are separate evidence. `bin/check-transient-feedback --development` may report absent evidence without failing. Release mode uses the default sibling showcase or `--showcase=PATH`. Both roots must be clean Git worktrees. The showcase must contain the canonical route plus the exact Task 6 capture component, view, and focused test paths. Before execution, the focused test must resolve `Native\Mobile\Testing\Native` (with ordinary aliases or a fully qualified name), visit exactly `/captures/transient-feedback`, assert the `Firstlight Transient Feedback` capture screen identity, and assert the single deterministic `FeedbackStore` record: success tone, `Appointment saved`, `transient-feedback-capture`, held lifetime, `Undo`, and `undo-save`. The exact manifest command then runs without a shell through a continuously drained process with a five-second timeout and a 4,000-byte combined diagnostic ceiling.

PNG validation reads no whole screenshot, combined IDAT payload, or complete decoded image into memory. Files are limited to 32 MiB, individual chunks to 16 MiB, portrait dimensions to 4096 by 8192 and 20 million pixels, and expected decoded scanlines to 64 MiB. The gate checks every chunk CRC, streams consecutive IDAT bytes through incremental zlib inflate, enforces the exact scanline count and filter bytes for either non-interlaced data or all seven Adam7 passes, and requires an empty IEND at EOF. A compressed stream that expands beyond the format-derived scanline count is rejected before it can act as a zip bomb.
...
Missing or duplicate rows, wrong screenshot paths, placeholder hashes, open checklist items, and empty or one-character evidence are release-blocking. Placeholder words are matched case-insensitively as complete words anywhere in identity values, reviewer/date/targets, screenshot details, or evidence details; this includes `PLACEHOLDER`, `TBD`, `TBC`, `UNKNOWN`, `OPEN`, `PENDING`, `DEFERRED`, `NOT RUN`, `SKIP`, `SKIPPED`, `TODO`, `FIXME`, `SAMPLE`, `FAKE`, `DUMMY`, `BLOCKED`, `FAIL`, and `FAILED`. Word matching does not reject incidental substrings such as “sampled”. The future review file is not created until real Task 8 evidence exists.
