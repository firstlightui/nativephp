...
| II. Familiar and coherent APIs | PASS | `Firstlight::feedback()` exposes stable ID, message, four semantic tones, optional single action, automatic or held lifetime, replace-by-ID, remove, and clear. Validation rejects blank or incomplete authored data. The API excludes arbitrary duration, position, styling, rich content, multiple actions, and external queue mutation. |
