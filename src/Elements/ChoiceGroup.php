...
class ChoiceGroup extends Element
{
    protected string $type = 'firstlight.choice-group';
    ...
    public function value(mixed $value): static
    {
        if (! is_string($value) && ! is_int($value) && ! is_array($value) && $value !== null) {
            throw new InvalidArgumentException(
                'Choice Group value must be a string, integer, array, or null.'
            );
        }
    ...
    public function syncMode(string $mode): static
    {
        if ($mode !== 'live') {
            throw new InvalidArgumentException(
                "Choice Group commits immediately, so the `{$mode}` sync mode has no effect. "
    ...
    private function optionCallbacks(
        ...
        return array_map(function (NormalizedOption $option) use ($registry, $selected, $disabled): string {
            if (
                ...
                || (! $this->multiple && $this->contains($selected, $option->value))
            ) {
                return '0';
            }
    ...
    private function normalizeSelectedValues(NormalizedOptions $options): array
    {
        if (! $this->multiple) {
            if (is_array($this->rawValue)) {
                throw new InvalidArgumentException(
                    'Choice Group single-choice value must be a string, integer, or null.'
                );
            }
        ...
        if (! is_array($this->rawValue)) {
            throw new InvalidArgumentException(
                'Choice Group multiple-choice value must be an array or null.'
            );
        }

        if (! array_is_list($this->rawValue)) {
            throw new InvalidArgumentException(
                'Choice Group multiple-choice value must be a list.'
            );
        }
        ...
        foreach ($this->rawValue as $index => $value) {
            if (! is_string($value) && ! is_int($value)) {
                throw new InvalidArgumentException(
                    "Choice Group selected value at index [{$index}] must be a string or integer."
                );
            }
            ...
            if ($valueType !== null && $valueType !== $type) {
                throw new InvalidArgumentException(
                    'Choice Group selected values cannot mix string and integer values.'
                );
            }
            ...
            if (isset($seen[$key])) {
                throw new InvalidArgumentException(
                    "Choice Group has a duplicate selected value [{$key}]."
                );
            }
    ...
    private function assertSelectedTypeMatchesOptions(array $selected, NormalizedOptions $options): void
    {
        ...
        if ($selectedType !== $options->valueType) {
            throw new InvalidArgumentException(
                "Choice Group selected value type [{$selectedType}] must match option value type [{$options->valueType}]."
            );
        }
    }
    ...
    private function assertCompatibleAttributes(array $attrs): void
    {
        foreach (self::INCOMPATIBLE_ATTRIBUTES as $attribute) {
            if (array_key_exists($attribute, $attrs)) {
                throw new InvalidArgumentException(
                    "Firstlight Choice Group `{$attribute}` is not supported."
                );
            }
        }
    }
