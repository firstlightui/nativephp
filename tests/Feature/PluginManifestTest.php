...
it('declares the Firstlight package identity', function () {
    ...
    expect($composer['name'])->toBe('firstlightui/nativephp')
        ...
        ->and($composer['require']['nativephp/mobile-ui'])->toBe('^0.4')
...
it('registers the production component catalogue with canonical renderers', function () {
    ...
    expect($manifest['components'])->toBe([
        ...
        [
            'type' => 'firstlight.callout',
            'element' => 'FirstlightUI\\Elements\\Callout',
            'blade' => 'FirstlightUI\\Components\\Callout',
            'android_renderer' => 'dev.firstlightui.plugins.firstlight_ui.ui.CalloutRenderer',
            'ios_renderer' => 'CalloutRenderer',
            'self_closing' => true,
        ],
        [
        ...
        [
            ...
            'android_renderer' => 'dev.firstlightui.plugins.firstlight_ui.ui.ConfirmationDialogRenderer',
            'ios_renderer' => 'ConfirmationDialogRenderer',
            'self_closing' => true,
        ],
    ]);
});
...
it('registers Confirmation Dialog as paired Firstlight renderers rather than an adapter', function () {
    $manifest = json_decode(file_get_contents(dirname(__DIR__, 2).'/nativephp.json'), true, flags: JSON_THROW_ON_ERROR);
    $component = collect($manifest['components'])->firstWhere('type', 'firstlight.confirmation-dialog');
